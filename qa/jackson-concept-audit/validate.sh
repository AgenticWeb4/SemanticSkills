...
SKILL_DIR="$(cd "$QA_DIR/../../skills/jackson-concept-audit" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: jackson-concept-audit$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$jackson-concept-audit' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
}
...
printf 'OK: jackson-concept-audit validation passed\n'
