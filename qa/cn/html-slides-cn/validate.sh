...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/html-slides-cn" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: html-slides-cn$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$html-slides-cn' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
...
printf 'OK: html-slides-cn validation passed\n'
