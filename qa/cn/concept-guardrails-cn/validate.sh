...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/concept-guardrails-cn" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: concept-guardrails-cn$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  ! rg -q '^license:' "$SKILL_DIR/SKILL.md" || fail "SKILL.md must omit license to avoid ClawHub MIT-0 conflict"
  rg -q '\$concept-guardrails-cn' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
...
check_references() {
  local ref
  for ref in spec-format audit concept drift-detection pipeline sync map hooks-runtime; do
...
printf 'OK: concept-guardrails-cn validation passed\n'
