...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/concept-audit-cn" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: concept-audit-cn$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$concept-audit-cn' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
  rg -q '^  allow_implicit_invocation: false$' "$SKILL_DIR/agents/openai.yaml" || fail "Codex implicit invocation must be disabled"
}

check_references() {
  local ref
  for ref in spec-format drift-checklist composition-checklist sources; do
...
printf 'OK: concept-audit-cn validation passed\n'
