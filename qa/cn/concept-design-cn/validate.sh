...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/concept-design-cn" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: concept-design-cn$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$concept-design-cn' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
...
check_references() {
  local ref
  for ref in spec-format criteria sync-notation example-reserving sources; do
...
printf 'OK: concept-design-cn validation passed\n'
