...
SKILL_DIR="$(cd "$QA_DIR/../../skills/jackson-concept-implementation" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: jackson-concept-implementation$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$jackson-concept-implementation' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
}
...
printf 'OK: jackson-concept-implementation validation passed\n'
