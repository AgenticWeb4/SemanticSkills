...
实现开始时把规格迁到代码旁（`jackson-concept-prd` 暂存于集中 PRD 目录的部分）：每概念 CONCEPT.md 进模块目录、SYNCS.md 进 syncs 目录。此后**规格先行**：先改规格再改实现——共存规格即 wyx 架构护栏（`wyx:concept drift`、边界注入）的扫描对象，也是每个概念可独立重生成的 prompt（概念独立 = 生成时上下文不必带其他概念）。
