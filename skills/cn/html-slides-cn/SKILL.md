---
name: html-slides-cn
description: "Builds a PPT / 胶片 / 汇报 / 授课 deck as a plain single-file 1920×1080 HTML in a pluggable style (Huawei 华为 official light/dark, Apple, generic), confirming outline and per-page table first; optional PPTX export. Use when the user explicitly invokes $html-slides-cn; not for general slide-design questions."
compatibility: File read/write only. Optional PPTX export needs Python 3, playwright and python-pptx (never auto-installed).
metadata:
  language: zh-CN
  translation_of: html-slides
  author: ontology-of-everything
  version: "0.1.1"
  openclaw:
    homepage: https://github.com/ontology-of-everything/concept-skills/tree/main/skills/cn/html-slides-cn
---
...
**仅显式调用**：用户写 `$html-slides-cn`（或同名斜杠命令）或点名本技能时启用；泛泛聊 PPT 不触发。不做的事：不代用户编造数据与引文；不安装依赖；PPTX 只在用户要求时导出并明说是哪一档（见输出格式）。
