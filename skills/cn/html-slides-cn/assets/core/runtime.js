/* html-slides-cn · deck 运行时（≤120 行，无依赖）。整段复制进 deck 末尾的 <script>。
...
(() => {
  ...
  const key = 'html-slides-cn:' + location.pathname, q = new URLSearchParams(location.search);
  ...
  if (q.has('debug')) {
    ...
    console.log(bad.length ? `[html-slides-cn debug] ${bad.length} overflow issue(s):\n` + bad.join('\n') : '[html-slides-cn debug] no overflow');
